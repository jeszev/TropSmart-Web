<template>
  ...
  >
              ...
              <v-container>
                <v-row>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field
                      v-model="cargoInput.customerFirstName"
                      label="Correo"
                    ></v-text-field>
                  </v-col>
                </v-row>
    ...
    <template v-slot:[`item.actions`]="{ item }">
      <v-btn small class="success " left="left" @click="cargoManager(item)">
        <span class="mr-2">Confirmar</span>
        <v-icon> mdi-send </v-icon>
      </v-btn>
      <v-btn class="ml-5 error" small @click="rejectItem(item)">
...
export default {
  data: () => ({
    ...
    select: null,
    filterSelected: "",
    items: ["Todos", "Solicitados", "Confirmados", "Terminados"],
    headers: [],
    cargoes: [],
    currentIndex: -1,
    cargoInput: {
  ...
  methods: {
    ...
    retrieveCargoes() {
      TsDataService.getAllCargoesByCustomerId(
        this.$store.state.auth.user.roleId
      ).then((response) => {
    ...
    retrieveRequestedCargoesByDriver() {
      TsDataService.getRequestedCargoesByDriverId(
        this.$store.state.auth.user.roleId
      ).then((response) => {
        this.cargoes = response.data.resourceList;
        console.log(this.cargoes);
    ...
    retrieveConfirmedCargoesByDriver() {
      TsDataService.getConfirmedCargoesByDriverId(
        this.$store.state.auth.user.roleId
      ).then((response) => {
        this.cargoes = response.data.resourceList;
        console.log(this.cargoes);
    ...
    retrieveFinishedCargoesByDriver() {
      TsDataService.getFinishedCargoesByDriverId(
        this.$store.state.auth.user.roleId
      ).then((response) => {
        this.cargoes = response.data.resourceList;
        console.log(this.cargoes);
    ...
    UserInfo(item) {
      this.selectDriver.driverFirstName = this.cargoes.indexOf(
        item
      ).driverFirstName;
      this.selectDriver.driverLastName = this.cargoes.indexOf(
        item
      ).driverLastName;
      this.dialog = true;
    },
    ...
    passSelection(data) {
      ...
      if (data == "Todos") this.retrieveCargoesByDriver();
      if (data == "Solicitados") this.retrieveRequestedCargoesByDriver();
    ...
    cargoManager(data) {
      ...
      firebase.analytics().logEvent("page_notification", { name: "confirmed" });
      ...
      if (data.cargoStatus == "In process") {
        ...
        TsDataService.setCargoDeliver(data.id).then((response) => {
          this.retrieveCargoes();
    ...
    rejectItem(data) {
      ...
      firebase.analytics().logEvent("page_notification", { name: "declined" });
      TsDataService.setCargoReject(data.id).then((response) => {
        this.retrieveCargoes();
